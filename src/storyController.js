...
var topicprogress = "NotGenerating"

var suggestTopic = async (req, res) => {
    try {
        await client.connect();

        var results = await client.db("Director").collection("suggested_topics").insertOne({
...
var generateStory = async () => {
    ...
    try {
        ...
        var count = await client.db("Director").collection("suggested_topics").countDocuments();

         if (count == 0) {
            generating = false;
            console.log("No suggested topics available. Convince people to send in topics!");
        ...
        //Retrieve all
        var results = await client.db("Director").collection("suggested_topics").find().toArray();
        ...
        if (results.length != 0) {
            ...
            await client.db("Director").collection("suggested_topics").deleteOne({
                _id: results[0]._id
            });
...
var getModelId = (character) => {
    ...
    switch (character.toLowerCase()) {
        ...
        case "squidward":
      	...
      	case "plankton":
        	return config.fakeYou.plankton
        default:
            return config.fakeYou.spongebob;
    }
}
...
var autoGenerateTopic = async () => {
    try {
        ...
        await client.db("Director").collection("suggested_topics").insertOne({
            requestor_id: "The Director",
            priority: -1,
            topic: topic,
            date: new Date()
        });
...
var validateCharacters = (preProcessed) => {
    const validCharacters = ["Spongebob", "Patrick", "Squidward", "Plankton"];
